...
% TODO: Add actual benchmark numbers when available
\subsection{Results}
\label{subsec:comparison_results_chirho}

\begin{table}[ht]
\centering
\caption{Comparison with differentiable logic systems (preliminary)}
...
Path Query & Ours & -- & -- & -- \\
           & Scallop & -- & -- & -- \\
\midrule
Type Infer & Ours & 15 $\mu$s & 100\% & 1 MB \\
           & Baseline & -- & -- & -- \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Note}: Full Scallop/DeepProbLog benchmarks are in progress.
Preliminary type inference results (Section~\ref{sec:type_infer_chirho}) demonstrate the approach.
...
\begin{itemize}
  \item GPU-accelerated differentiable operations
  \item Full Scallop/DeepProbLog benchmark suite
  \item Integration with neural network architectures (transformers, GNNs)
  \item Quantization of soft probabilities to $k$-bit for efficient inference
\end{itemize}
