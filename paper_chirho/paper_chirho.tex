...
\begin{abstract}
We observe that miniKanren's relational search over finite domains can be represented as sparse Boolean tensor operations.
The key insight is simple: variable domains become bitmasks, and unification becomes bitwise AND.
...
We implement this approach in Rust (production) with verified FPGA targets (Calyx IR, Clash).
...
For infinite domains, we use hash consing to generate terms on demand while maintaining the bit-parallel representation.
\end{abstract}
