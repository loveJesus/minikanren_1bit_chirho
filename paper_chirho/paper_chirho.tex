...
Table~\ref{tab:solvers} compares our Python prototype against Z3, clingo, and kanren.
...
Table~\ref{tab:apps} shows our production Rust implementation performance.
...
\subsection{Hardware vs Reference miniKanren}

Table~\ref{tab:hw-vs-ref} compares our bit-parallel hardware implementation against a traditional stream-based miniKanren.

\begin{table}[H]
\centering
\begin{tabular}{lrrr}
\toprule
\textbf{Operation} & \textbf{Hardware (1-bit)} & \textbf{Streams} & \textbf{Speedup} \\
\midrule
Simple unify & 40 ns & 271 ns & 6.8$\times$ \\
Conjunction chain (2) & 78 ns & 514 ns & 6.6$\times$ \\
Conjunction chain (4) & 144 ns & 1.1 $\mu$s & 7.6$\times$ \\
Conjunction chain (8) & 288 ns & 2.2 $\mu$s & 7.6$\times$ \\
\bottomrule
\end{tabular}
\caption{Bit-parallel goals vs stream-based goals}
\label{tab:hw-vs-ref}
\end{table}
