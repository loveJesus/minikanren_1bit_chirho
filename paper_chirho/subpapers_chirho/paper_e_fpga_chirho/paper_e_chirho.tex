...
at $\sim$8\% utilization. Timing closure on physical hardware remains pending.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Timing Unverified}: While we now have synthesis results from Yosys,
    physical place-and-route and timing closure on actual FPGA hardware remain pending.
    Vendor-specific synthesis (Quartus, Vivado) may yield different results.
...
\begin{enumerate}
  \item \textbf{Physical Synthesis}: Complete Vivado synthesis flow and report
    actual resource utilization and timing.
...
with modest resource consumption (12\% LUT, 27\% register on Artix-7). These
results are \emph{simulation-based}; physical synthesis remains future work.
