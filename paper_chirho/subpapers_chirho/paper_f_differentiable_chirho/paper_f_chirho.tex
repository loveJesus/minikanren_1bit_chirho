...
\paragraph{Three Modes of Operation.}
We distinguish three complementary modes, each suited to different tasks:

\begin{enumerate}
  \item \textbf{Exact Boolean/semiring algebra} (tensor contraction): Deterministic computation---prove constraints or compute weights exactly. This is the foundation (Papers A--E).

  \item \textbf{Differentiable relaxation} (soft semiring + Gumbel-softmax): For \emph{training} heuristics and neural-guided choices. Fast gradients but approximate semantics away from Boolean endpoints. \emph{This paper's primary focus.}

  \item \textbf{Probabilistic inference by sampling} (Trace MH / HMC over random streams): For unbiased probabilistic semantics and continuous variables. Hamiltonian Monte Carlo is where AD/tensor machinery truly shines---future work (Section~\ref{sec:sampling_inference_chirho}).
\end{enumerate}
...
\subsection{Gumbel-Softmax for Discrete Choices (Training Mode)}
\label{subsec:gumbel_chirho}

Gumbel-softmax is a \emph{training} technique: it enables gradient-based learning of discrete choices.
It does \emph{not} provide unbiased probabilistic inference---for that, see Section~\ref{sec:sampling_inference_chirho}.
...
% ============================================================================
\section{Sampling-Based Inference (Future Work)}
\label{sec:sampling_inference_chirho}
% ============================================================================

Gumbel-softmax is a \emph{training} technique: it provides biased gradients for learning discrete parameters.
For \emph{unbiased probabilistic inference}---computing posterior distributions over program outputs---we need sampling methods.

\subsection{Trace-Based MCMC}
\label{subsec:trace_mcmc_chirho}

Following lightweight probabilistic programming approaches~\cite{lightweight_mcmc_chirho}, we can view a logic program's random choices as a \emph{trace}:
\[
\text{trace} = [(c_1, v_1), (c_2, v_2), \ldots, (c_n, v_n)]
\]
where $c_i$ is a choice point (e.g., branch in \texttt{conde}) and $v_i$ is the selected value.

\textbf{Metropolis-Hastings} over traces:
\begin{enumerate}
  \item Run program with current trace, computing likelihood
  \item Propose new trace by modifying one choice
  \item Accept/reject based on likelihood ratio
\end{enumerate}

This provides unbiased samples from the posterior over program outputs given observations.

\subsection{Hamiltonian Monte Carlo for Continuous Variables}
\label{subsec:hmc_chirho}

For programs with continuous variables (e.g., soft domain weights), Hamiltonian Monte Carlo (HMC) is preferable.
HMC uses gradient information to propose moves, achieving much better mixing than random-walk MH.

\textbf{Key insight}: The tensor contraction view makes HMC natural.
The log-probability of a solution is a differentiable function of continuous parameters (soft weights).
AD through tensor contraction provides the gradients HMC needs.

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Method} & \textbf{Use Case} & \textbf{AD Required?} \\
\midrule
Gumbel-softmax & Training discrete choices & Yes (for $\nabla$) \\
Trace MH & Discrete probabilistic inference & No \\
HMC & Continuous probabilistic inference & Yes (for $\nabla \log p$) \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Connection to Guanxi}
\label{subsec:guanxi_chirho}

The tensor contraction framework connects to Guanxi-style constraint propagation:
\begin{itemize}
  \item \textbf{Domains as distributions}: Instead of hard bitmasks, probability distributions over values
  \item \textbf{Constraints as soft factors}: Instead of Boolean satisfaction, likelihood functions
  \item \textbf{Inference as marginalization}: Tensor contraction computes marginal distributions
\end{itemize}

HMC over the continuous relaxation provides a principled way to sample from the posterior---this is where the AD/tensor machinery of this paper truly shines.

\textbf{Status}: Trace MH and HMC are \emph{future work}. The current implementation focuses on Gumbel-softmax training (Sections~\ref{subsec:gumbel_chirho}--\ref{subsec:ste_chirho}).
...
\paragraph{Evidence and Artifacts.}
\begin{itemize}
  \item \textbf{Source code}: \texttt{clash\_chirho/DiffTrainChirho.hs} (820 lines)
  \item \textbf{Synthesis report}: \texttt{synth\_chirho/aws\_f2\_chirho\_cl/reports/synthesis\_chirho.rpt}
  \item \textbf{Benchmark script}: \texttt{synth\_chirho/aws\_f2\_chirho\_cl/benchmarks\_chirho/diff\_train\_bench\_chirho.py}
  \item \textbf{CPU baseline}: Rust implementation in \texttt{rust\_chirho/src/diff\_semiring\_chirho.rs}
\end{itemize}

\textbf{What was measured}: CPU times are Rust \texttt{criterion} benchmarks. FPGA times are cycle counts from simulation multiplied by clock period (4ns @ 250MHz). Speedup = CPU time / FPGA time.
...
\textbf{Probabilistic Programming and Sampling.}
Lightweight MCMC~\cite{lightweight_mcmc_chirho} introduced trace-based inference for probabilistic programs.
Hamiltonian Monte Carlo~\cite{hmc_chirho} uses gradients for efficient sampling.
Our tensor framework provides gradients naturally, making HMC a promising direction (Section~\ref{sec:sampling_inference_chirho}).
