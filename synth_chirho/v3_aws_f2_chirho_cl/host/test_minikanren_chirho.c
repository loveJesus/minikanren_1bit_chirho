/**
 * For God so loved the world - John 3:16 ☧
 *
 * Host test software for miniKanren FPGA accelerator
 * Tests basic functionality via AWS FPGA APIs
 */

#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
#include <unistd.h>
#include <fcntl.h>
#include <errno.h>
#include <fpga_pci.h>
#include <fpga_mgmt.h>
#include <fpga_dma.h>

// Register addresses (must match cl_minikanren_chirho.sv)
#define REG_VERSION_CHIRHO      0x000  // Version register
#define REG_STATUS_CHIRHO       0x004  // Status register
#define REG_CONTROL_CHIRHO      0x008  // Control register
#define REG_CMD_CHIRHO          0x010  // Command register
#define REG_RESP_LO_CHIRHO      0x014  // Response low 32 bits
#define REG_RESP_HI_CHIRHO      0x018  // Response high 32 bits
#define REG_HBM_TEST_CHIRHO     0x020  // HBM test register

// Expected values
#define EXPECTED_VERSION_CHIRHO 0xF216316A  // F2, John 3:16, rev A

// Commands (must match searchEngineChirho command encoding)
#define CMD_NOP_CHIRHO          0x00
#define CMD_INIT_CHIRHO         0x01
#define CMD_STEP_CHIRHO         0x02
#define CMD_READ_CHIRHO         0x03

static int slot_id_chirho = 0;
static pci_bar_handle_t pci_bar_handle_chirho = PCI_BAR_HANDLE_INIT;

int check_afi_ready_chirho(void) {
    struct fpga_mgmt_image_info info_chirho;
    int rc_chirho;

    rc_chirho = fpga_mgmt_describe_local_image(slot_id_chirho, &info_chirho, 0);
    if (rc_chirho) {
        printf("ERROR: Unable to get AFI information. rc=%d\n", rc_chirho);
        return -1;
    }

    if (info_chirho.status != FPGA_STATUS_LOADED) {
        printf("ERROR: AFI not loaded. Status: %d\n", info_chirho.status);
        printf("Load AFI first with: fpga-load-local-image -S %d -I <agfi-id>\n", slot_id_chirho);
        return -1;
    }

    printf("AFI loaded successfully.\n");
    printf("  Vendor ID: 0x%04x\n", info_chirho.spec.map[FPGA_APP_PF].vendor_id);
    printf("  Device ID: 0x%04x\n", info_chirho.spec.map[FPGA_APP_PF].device_id);

    return 0;
}

int test_version_chirho(void) {
    uint32_t version_chirho;
    int rc_chirho;

    printf("\n=== Test 1: Version Register ===\n");

    rc_chirho = fpga_pci_peek(pci_bar_handle_chirho, REG_VERSION_CHIRHO, &version_chirho);
    if (rc_chirho) {
        printf("FAIL: Unable to read version register. rc=%d\n", rc_chirho);
        return -1;
    }

    printf("Version register: 0x%08x\n", version_chirho);

    if (version_chirho == EXPECTED_VERSION_CHIRHO) {
        printf("PASS: Version matches expected 0x%08x\n", EXPECTED_VERSION_CHIRHO);
        return 0;
    } else {
        printf("FAIL: Expected 0x%08x, got 0x%08x\n", EXPECTED_VERSION_CHIRHO, version_chirho);
        return -1;
    }
}

int test_status_chirho(void) {
    uint32_t status_chirho;
    int rc_chirho;

    printf("\n=== Test 2: Status Register ===\n");

    rc_chirho = fpga_pci_peek(pci_bar_handle_chirho, REG_STATUS_CHIRHO, &status_chirho);
    if (rc_chirho) {
        printf("FAIL: Unable to read status register. rc=%d\n", rc_chirho);
        return -1;
    }

    printf("Status register: 0x%08x\n", status_chirho);
    printf("  Engine ready: %s\n", (status_chirho & 0x1) ? "YES" : "NO");
    printf("  HBM ready:    %s\n", (status_chirho & 0x2) ? "YES" : "NO");
    printf("  Busy:         %s\n", (status_chirho & 0x4) ? "YES" : "NO");

    return 0;
}

int test_command_chirho(void) {
    uint32_t cmd_chirho, resp_lo_chirho, resp_hi_chirho;
    int rc_chirho;

    printf("\n=== Test 3: Command/Response ===\n");

    // Send NOP command
    cmd_chirho = CMD_NOP_CHIRHO;
    printf("Sending NOP command (0x%02x)...\n", cmd_chirho);

    rc_chirho = fpga_pci_poke(pci_bar_handle_chirho, REG_CMD_CHIRHO, cmd_chirho);
    if (rc_chirho) {
        printf("FAIL: Unable to write command register. rc=%d\n", rc_chirho);
        return -1;
    }

    // Small delay for command processing
    usleep(1000);

    // Read response
    rc_chirho = fpga_pci_peek(pci_bar_handle_chirho, REG_RESP_LO_CHIRHO, &resp_lo_chirho);
    rc_chirho |= fpga_pci_peek(pci_bar_handle_chirho, REG_RESP_HI_CHIRHO, &resp_hi_chirho);
    if (rc_chirho) {
        printf("FAIL: Unable to read response registers. rc=%d\n", rc_chirho);
        return -1;
    }

    printf("Response: 0x%08x_%08x\n", resp_hi_chirho, resp_lo_chirho);
    printf("PASS: Command/response path working\n");

    return 0;
}

int test_hbm_chirho(void) {
    uint32_t test_val_chirho, read_val_chirho;
    int rc_chirho;

    printf("\n=== Test 4: HBM Access ===\n");

    // Write test pattern to HBM test register
    test_val_chirho = 0xDEADBEEF;
    printf("Writing test pattern 0x%08x to HBM test register...\n", test_val_chirho);

    rc_chirho = fpga_pci_poke(pci_bar_handle_chirho, REG_HBM_TEST_CHIRHO, test_val_chirho);
    if (rc_chirho) {
        printf("FAIL: Unable to write HBM test register. rc=%d\n", rc_chirho);
        return -1;
    }

    usleep(1000);

    rc_chirho = fpga_pci_peek(pci_bar_handle_chirho, REG_HBM_TEST_CHIRHO, &read_val_chirho);
    if (rc_chirho) {
        printf("FAIL: Unable to read HBM test register. rc=%d\n", rc_chirho);
        return -1;
    }

    printf("Read back: 0x%08x\n", read_val_chirho);

    if (read_val_chirho == test_val_chirho) {
        printf("PASS: HBM loopback test passed\n");
        return 0;
    } else {
        printf("WARN: HBM loopback mismatch (may be expected if HBM write-through)\n");
        return 0;  // Not a hard failure
    }
}

void print_usage_chirho(const char *prog_chirho) {
    printf("Usage: %s [-s slot_id]\n", prog_chirho);
    printf("  -s slot_id  FPGA slot to use (default: 0)\n");
}

int main(int argc, char *argv[]) {
    int rc_chirho;
    int opt_chirho;
    int tests_passed_chirho = 0;
    int tests_failed_chirho = 0;

    printf("============================================\n");
    printf("miniKanren FPGA Test ☧\n");
    printf("For God so loved the world - John 3:16\n");
    printf("============================================\n\n");

    // Parse arguments
    while ((opt_chirho = getopt(argc, argv, "s:h")) != -1) {
        switch (opt_chirho) {
            case 's':
                slot_id_chirho = atoi(optarg);
                break;
            case 'h':
            default:
                print_usage_chirho(argv[0]);
                return (opt_chirho == 'h') ? 0 : 1;
        }
    }

    printf("Using FPGA slot: %d\n", slot_id_chirho);

    // Initialize FPGA management library
    rc_chirho = fpga_mgmt_init();
    if (rc_chirho) {
        printf("ERROR: Unable to initialize FPGA management library. rc=%d\n", rc_chirho);
        return 1;
    }

    // Check AFI is loaded
    rc_chirho = check_afi_ready_chirho();
    if (rc_chirho) {
        goto cleanup;
    }

    // Attach to FPGA
    rc_chirho = fpga_pci_attach(slot_id_chirho, FPGA_APP_PF, APP_PF_BAR0, 0, &pci_bar_handle_chirho);
    if (rc_chirho) {
        printf("ERROR: Unable to attach to FPGA. rc=%d\n", rc_chirho);
        goto cleanup;
    }

    printf("Attached to FPGA successfully.\n");

    // Run tests
    if (test_version_chirho() == 0) tests_passed_chirho++; else tests_failed_chirho++;
    if (test_status_chirho() == 0) tests_passed_chirho++; else tests_failed_chirho++;
    if (test_command_chirho() == 0) tests_passed_chirho++; else tests_failed_chirho++;
    if (test_hbm_chirho() == 0) tests_passed_chirho++; else tests_failed_chirho++;

    // Summary
    printf("\n============================================\n");
    printf("Test Summary\n");
    printf("============================================\n");
    printf("Passed: %d\n", tests_passed_chirho);
    printf("Failed: %d\n", tests_failed_chirho);
    printf("\n");

    if (tests_failed_chirho == 0) {
        printf("ALL TESTS PASSED ☧\n");
        printf("Soli Deo Gloria\n");
    } else {
        printf("SOME TESTS FAILED\n");
    }

cleanup:
    if (pci_bar_handle_chirho != PCI_BAR_HANDLE_INIT) {
        fpga_pci_detach(pci_bar_handle_chirho);
    }
    fpga_mgmt_close();

    return tests_failed_chirho > 0 ? 1 : 0;
}
